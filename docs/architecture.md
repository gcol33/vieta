...
## 13. The remaining surface

Layer A settles identity: what a term is, and when two terms are the same. The
store answers nothing about what a term means, what may be done with it, or which
of the many equivalent forms of an answer to show. Those are three further systems,
and most of their content is already registered.
...
| Assumptions and obligations | D3, D4, §2.5, decided |
| Relations above term identity | D36 bounds what Layer A may hold, §4 and D13 place the rest |
| Evaluation semantics | D25, D20, D21, D30, D33 |
| Compiled symbolic matching | D14 contract open, D35 matcher handoff open |
...
D6 asks for an enumeration and offers one: lexical function arguments, integration
variables, summation indices, pattern variables, and sequence-pattern variables
bind; global symbols, domain generators, and generated indeterminates do not. A
seven-kind taxonomy of variables lands on exactly those cases, one for one, which
is evidence the enumeration is stable rather than provisional. Three cases sit
outside it.

**Quantified variables inside a proposition.** §2.5's three tiers take free
variables whose assumptions hold at the context. A proposition is also a term, and
`forall n. n >= 1 => f(n) > 0` binds `n` inside it. The tier-2 solver has to know
that this `n` is not the ambient `n`, or it will discharge a condition about one
variable using a fact about another. Whether Vieta's assumption contexts accept
quantified propositions at all is a separate decision, and it is one D6 forces,
because the answer changes what the traversal API has to go under.

**Comprehension and index-set variables.** `{x in S : P(x)}` binds `x`. So does the
index in `Sum(a(i), i in I)` once the index set is itself a term rather than a
range. This is the dummy-variable case D6 already covers, reached through a
different surface form, and it is worth naming because the binder sits in argument
position under a head that also takes ordinary arguments.

**Constants generated by a solution set.** D6 places generated indeterminates among
the non-binders. That is right for a domain generator and unresolved for the
arbitrary constant in an ODE solution or the parameter in `{x = 2*pi*k : k in Z}`.
The choice is whether such a constant is a fresh global symbol or a variable bound
by the solution set, and the two differ observably: under the second, two solution
...
D27 defines `simplify` as a preferable equivalent form under a context and a cost
function, and §4 claims that extraction under a cost function dissolves the
expand-versus-factor dilemma. Both are right about the mechanism and silent about
the objective. Nothing in the documents says what a cost function ranges over.

`(x + 1)^5` has an expanded form, a factored form, a Horner form, a numerically
stable evaluation order, a coefficient vector, and itself. There is no simplest one.
Two different requests reach that set:

- *Undirected.* Find the best equivalent under a metric. The user supplies a
  preference and accepts whatever wins. This is extraction, and D27 already owns it.
- *Directed.* Return the factored form. The user supplies a target and the metric
  is irrelevant. Reaching a named target by tuning a cost function until it wins is
  an indirect way to state something direct.

D27's five operations have no slot for the second. The consequence is predictable:
`simplify` acquires an option per target form, and D27 names one operation doing
several jobs as the failure it exists to prevent. A sixth operation is the obvious
answer, `convert(expr, form)`, separable from `simplify` because their return
conditions differ. Extraction always returns something, since the input is in its
own equivalence class. A directed conversion can be `Unable`, which is a `Status`
(D4), and a target that is unreachable is ordinary rather than exceptional.

A third consumer makes the objective vocabulary a question of its own.
Stability-aware compilation (§11.1, M7) takes an objective that is neither a
simplification metric nor a target form. So there is one vocabulary spanning
extraction costs, conversion targets, and compilation objectives, or there are
three that will be confused for each other in the surface syntax.

Whatever it turns out to be, **it is a value**, by D13's argument. A global mutable
notion of which form is simplest is the same unsound-caching, uncompilable
construct as a global mutable rule table, and it is worse for reproducibility,
because a changed cost function changes answers silently while a changed rule table
at least changes what applies.

### 13.3 Evidence that raises confidence without discharging anything

Three mechanisms carry how much a result can be trusted. `Status` (D4) reports the
operational outcome. An obligation (D3) carries conditional validity. A derivation
(D24) records how the result was reached and is replayable. A candidate answer
checked numerically at a hundred points is none of the three. It is not a status,
because the operation succeeded. It is not an obligation, because nothing about it
is conditional in the sense a solver could discharge. It is not a derivation,
because the steps that produced the answer are not what the numeric check
establishes.

Three ways to place it:

1. An undischarged obligation stating the identity that was checked and not proved,
   with the evidence as a derivation node.
2. A confidence field on the result, which is a fourth channel alongside the three.
3. A `Status` variant.

The first costs no new vocabulary, keeps §2.5's rule that an answer with attached
conditions is a correct answer, and puts the hundred sample points where a
certificate belongs. The second is the one to avoid, because a number invites
arithmetic that means nothing when two of them combine. The third is wrong on its
face, since the operation did succeed.

The requirement that survives whichever is chosen: **a result whose obligations
were discharged by proof and a result whose obligations are open must not print
identically.** §11.1 claims checkable derivation graphs as genuinely novel, and the
printed distinction is the only place a user meets that claim.

### 13.4 A coercion is a rewrite, so its arrows can be partial

§11.2 trap 5 fixes the shape of coercion: a join in a lattice with a uniqueness
argument, and pairwise special cases forbidden. That settles which domain a mixed
operation lands in. It says nothing about the arrows, and the arrows are not
uniform:

```
Z    -> Q       total, injective
Q[x] -> Q(x)    total, injective
Q(x) -> Q[x]    partial, defined when the denominator divides
R    -> Q       partial, and not canonical when it succeeds
C    -> R       conditional on the imaginary part being zero, which is Richardson
```

The last line is the interesting one, because deciding it is exactly the
undecidable problem D4 exists to respect. So a coercion returns `Truth`-shaped
information and can fail to know. Which is to say a coercion is a D3 rewrite:
`NoMatch`, a guarded set, or a typed failure. Nothing new is required, and saying
it now is what stops coercion from growing a private error model that has to be
reconciled with `RewriteResult` later.

One consequence reaches §5 directly. Domain inference (step 2) computes a join over
descriptors and can succeed while the arrow realizing it emits an obligation, so a
successful inference does not make lowering (step 3) unconditional. §5 already
records that step 3 records its assumptions. This is why there are always
assumptions to record.

### 13.5 The seam has a sequence and no types

§5's six steps are the lower-compute-lift path, and they are a sequence of
paragraphs. A contract has to answer four things the paragraphs do not:

- What lowering returns when the target cannot represent the expression. §5 says
  failure is a normal outcome, which makes it a `Status` and not an error, and the
  descriptor that failed is part of the answer.
- Whether lifting is total. Every native object a domain can produce has to have a
  term image, or a computation can succeed and have no expressible result.
- What round-tripping guarantees. `lift(lower(t))` equal to `t` under domain
  equality and not under structural equality (D2), since lowering into `Q[x]` and
  back expands, and requiring structural equality would forbid the representation
  change that is the point of lowering.
- Whether the seam is one interface or one per domain family.

The fourth is the one that decides the other three, and D16 has already answered
the same question once, for relation theories: build the algebraic case, build the
differential case, extract the interface from two things that work. The seam is
the second interface with that shape, and it gets the same rule rather than a
fresh guess. What that leaves for now is the concrete `Q[x]` path in M2, written
without an abstraction over it, and the contract extracted when a second domain
family has run through it.
